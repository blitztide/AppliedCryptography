...
HashReturn SpongentHash(int data, DataLength databitlen, BitSequence *hashval)
{
	hashState  state;
	HashReturn res;
	...
	{
		read(data, state.messageblock, R_SizeInBytes);
#ifdef _PrintState_
	printf("\n");
	printf("m_i\t");
	for(int i = 0; i < R_SizeInBytes; i++) 
	...
	if(databitlen>0){
		read(data, state.messageblock, databitlen/8 + (databitlen%8?1:0));
